...
turbine_shaft_speed = abs(pump_shaft_speed(1))*gear_ratio; % rad/s
...
blade_chord_rotor = blade_width_rotor/sin(blade_chord_angle); % m - in an impulse rotor these are equal
...
blade_count_rotor =  4*round(2*pi*r_pitchline_rotor/blade_pitch_rotor/4); % unitless - number of blades along the circumference of the rotor
...
incidence_loss_coeff = interp1(incidence_loss_data(:,1), incidence_loss_data(:,2), incidence_angle_blade*180/pi); % unitless - NASA Turbines 1976; fig. 17
mach_loss_coeff = interp1(mach_loss_data(:,1), mach_loss_data(:,2), norm(M_blade_relative)); % unitless - NASA Turbines 1976; fig. 18
...
wheel_ratio = v_axial_rotor / norm(v_spouting); % unitless - ratio between outlet/inlet velocity of blades. Must be absolute, because the relative ratio would always be 1?
...
if blade_length_rotor < 1.5*A_choked/(blade_count_rotor*blade_opening_rotor)
    fprintf("Rotor blades are too short and choking the flow")
end

r_tip_rotor = r_pitchline_rotor + blade_length_rotor/2; % m
r_base_rotor = r_pitchline_rotor - blade_length_rotor/2; % m

%% Mass Flow Adjustments
mdot_total = mdot_cc + mdot_gg; % kg/s
gg_fraction = mdot_gg / mdot_total; % unitless
...
%% Rotor Stress
AN2 = pi*((r_tip_rotor-r_base_rotor)/0.0254)^2 * (turbine_shaft_speed*30/pi)^2;
...
r_tip_rotor/0.0254;
stress_radial = (3+rotor_poissons)/8*rotor_density*turbine_shaft_speed^2*r_tip_rotor^2; % Pa
stress_radial/6.895e+6; % ksi
stress_FSu = rotor_strength/stress_radial;

%% Other
thrust_turbine = mdot_gg * v_spouting(2); % N
