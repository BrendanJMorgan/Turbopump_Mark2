...
k1 = 1.2; 
k2 = 0.2334 + (pump_shaft_speed*r_eye_impeller/128.3)^4;
cavitation_limit = (pump_shaft_speed*r_eye_impeller)^2/(2*g); % unitless
...
ss_speed = pump_shaft_speed*sqrt(vdot_pump)/(g * NPSH_inducer)^0.75; % unitless - suction specific speed
...
r_tip_inducer = 0.5*0.3048*0.37843*(vdot_corrected*15850.323 / (pump_shaft_speed*30/pi*flow_coeff_inducer))^(1/3); % m
...
%% Blade Construction
v_inlet_inducer = [0; 0];
v_inlet_inducer(1) = pump_shaft_speed*r_tip_inducer; % m/s - tangential velocity at inducer pitchline tip
...
%% Head
v_tip_inducer = pump_shaft_speed*r_tip_inducer; % m/s
...
%% Other
inducer_torque = mdot_pump*pump_shaft_speed*r_tip_inducer^2; % N*m - this is a conservative maximum
