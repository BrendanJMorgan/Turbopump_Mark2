...
u_inlet = pump_shaft_speed*r_inlet;				% m/s - rotational speed at the inlet (U_1 in pump handbook)
...
v_blade_inlet = pump_shaft_speed*r_eye_impeller;    % m/s - blade tangential velocity at entrance (U_1)
v_blade_exit = pump_shaft_speed*r_exit_impeller;	% m/s - blade tangential velocity at exit (U_2 in pump handbook)
v_slip = slip_factor*v_blade_exit;			        % m/s - slip velocity (V_s in pump handbook)
v_merid_exit = vdot_pump/(2*pi*r_exit_impeller*w_exit);	% m/s - meridional velocity (V_m,2 in pump handbook)                                                       

hydraulic_efficiency = 1 - 0.071 / vdot_pump^0.25; % unitless - Jekat's Empirical Formula - valid for all specific speeds

v_tangential_entrance = 0; % m/s - assuming no pre-swirl from the inducer
v_tangential_exit = g*head_pump / (hydraulic_efficiency*v_blade_exit) + v_tangential_entrance; % m/s - this is Euler's pump equation rearranged (V_theta,2 in pump handbook). The tangential flow speed needed to achieve head requirement.                                                                                                                                                                                                                                                                                                                                                                                                                           
blade_angle_outlet = atan(v_merid_exit / (v_blade_exit*(1-slip_factor) - v_tangential_exit));    % rad - angle between blade tip azimuth and local tangential azimuth (beta_2 in pump handbook)
