...
[r_eye_impeller, fval, exitflag, output] = fzero(@(r_eye_impeller) eye_solver(r_eye_impeller, vdot_pump, pump_shaft_speed, eye_flow_coeff, r_eye_inner), [lower_bound, upper_bound], options);
...
eye_flow_coeff = vdot_pump/A_eye / (pump_shaft_speed*r_eye_impeller); % unitless - in case solver didn't exactly match input

%% The exit radius r2 (or diameter D2)
r_exit_impeller = 1 / pump_shaft_speed * sqrt (g*head_pump / head_coeff);

%% The exit width b2
w_exit = vdot_pump / (2*pi*pump_shaft_speed*r_exit_impeller^2*outlet_flow_coeff*blockage);
