...
% Pumps
pump_shaft_speed = 20000*pi/30;          % rad/s - angular velocity of the pump shaft, impeller, and inducers
...
% Turbine
r_pitchline_rotor = 0.060;      % m
gear_ratio = 3;               % unitless - higher makes for a faster, smaller turbine. 1 = common shaft
...
grid on
axis equal
plot(-shroud_curve_ox(:,1)*1000, shroud_curve_ox(:,2)*1000, 'color', 'cyan')
plot(-impeller_curve_ox(:,1)*1000, impeller_curve_ox(:,2)*1000, 'color', 'cyan')
plot(shroud_curve_fuel(:,1)*1000, shroud_curve_fuel(:,2)*1000, 'color', 'red')
plot(impeller_curve_fuel(:,1)*1000, impeller_curve_fuel(:,2)*1000, 'color', 'red')
line([0 0], ylim, 'color', 'green');  % x-axis
line(xlim, [0 0], 'color', 'green');  % y-axis
title("Impeller and Shroud Contours (mm)")
hold off
...
title('Impellers, Volutes, and Rotor (mm)');
grid on;
