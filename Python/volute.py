...
def volute(p) -> None:
    ...
    p.r_tongue = p.outlet_impeller * (1.03 + 0.0025 * p.specific_speed + 7e-8 * p.density_pump * p.head_pump)  # m - Gulich table 10.2, pg 729
    p.t_tongue = 0.04 * p.outlet_impeller  # m - Gulich pg 493
    p.A_throat = (p.vdot_pump / (3.8 * p.outlet_impeller * p.v_tangential_outlet) * (1 + np.sqrt(1 + 15.2 * p.outlet_impeller * p.v_tangential_outlet * (p.r_tongue + p.t_tongue) / p.vdot_pump))) ** 2  # m^2
    p.r_throat = p.outlet_impeller + 0.5 * np.sqrt(p.A_throat) + p.t_tongue  # m

    p.theta_volute = np.linspace(0.0, 2.0 * np.pi, 1000)  # rad
    p.A_volute = (p.vdot_pump * p.theta_volute / (7.6 * np.pi * p.outlet_impeller * p.v_tangential_outlet) * (1 + np.sqrt(1 + 30.4 * np.pi * p.outlet_impeller * p.r_tongue * p.v_tangential_outlet / (p.vdot_pump * p.theta_volute)))) ** 2  # m^2
    ...
    p.v_throat = p.vdot_pump / p.A_throat  # m/s - fluid outlet velocity into plumbing
