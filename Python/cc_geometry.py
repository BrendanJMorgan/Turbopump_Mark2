...
def cc_geometry():
    ...
    tca.A_throat = tca.A_throat  # m2 - Throat Area
    converge_angle = tca.converge_angle      # rad
    diverge_angle  = tca.diverge_angle       # rad

    # Cross Section Areas ------------------------------------------------------------------------------------------------
    r_throat = np.sqrt(tca.A_throat / np.pi)    # m - Throat Radius
    d_throat = 2 * r_throat                 # m - Throat Diameter
    A_exit   = tca.A_throat * tca.Ae_At         # m2 - Nozzle Exit Area
    d_exit   = np.sqrt(4 * A_exit / np.pi)  # m - Nozzle Exit Diameter
    ...
    # Chamber Length -----------------------------------------------------------------------------------------------
    dx = tca.dx # spatial step
    V_frustrum = np.pi/3 * (tca.r1_chamber**2 + tca.r1_chamber*r_throat + r_throat**2) * \
                 (tca.r1_chamber-r_throat)/np.tan(tca.converge_angle); # m3
    ...
    # Contours ----------------------------------------------------------------------------------------------------
    x1_throat = 0.5*(tca.d1_chamber-d_throat)/np.tan(tca.converge_angle) + l_chamber; 
        # m - Projected point, onto central axis, of the converging straight line contour IF throat had no curvature
    x2_throat = x1_throat + r_throat*(np.sin(tca.converge_angle)+1/np.tan(tca.converge_angle)*(np.cos(tca.converge_angle)-1));
        # m - Central point of the throat
    x3_throat = x2_throat + r_throat*(np.sin(tca.diverge_angle)+1/np.tan(tca.diverge_angle)*(np.cos(tca.diverge_angle)-1));
        # m - Projected point, onto central axis, of the diverging straight line contour IF throat had no curvature

    x_exit = x3_throat + 0.5*(d_exit-d_throat)/np.tan(diverge_angle); # Position of nozzle exit, aka length of the entire chamber + nozzle SUSSSSSSSSS
    tca.x = np.arange(0.0, x_exit + 0.5 * dx, dx) # m - positional domain

    # Define inner contour
    r1 = np.ones_like(tca.x) * 0.5 * d1_chamber # Chamber

    # Convergence
    index_l = int(np.floor(l_chamber/dx))
    r1[index_l:] = 0.5 * (d1_chamber - d_throat) * ( (tca.x[index_l:] - tca.x[index_l]) / (x1_throat - tca.x[index_l]) ) + 0.5 * d_throat # m

    # Divergence
    index_x3 = int(np.floor(x3_throat/dx))
    r1[index_x3:] = 0.5 * (d_exit - d_throat) * ( (tca.x[index_x3:] - tca.x[index_x3]) / (tca.x[-1] - tca.x[index_x3]) ) + 0.5 * d_throat # m

    # Throat Arc
    x_arc = -r_throat * np.sin(converge_angle) + np.arange(0, int(r_throat * np.sin(diverge_angle) / dx)) * dx
    throat_arc = 0.5 * d_throat - np.sqrt(r_throat**2 - x_arc**2) + r_throat
    index_throat = np.argmin(throat_arc)  # or your exact criterion
    index_x2 = int(np.ceil(x2_throat/dx))
    ...
    pipe_bound2 = int(len(tca.x) - 1 - np.argmax(mask_le[::-1])) if np.any(mask_le) else len(x) - 1
    ...
    # Cumulative Area ----------------------------------------------------------------------------------------
    drdx = np.gradient(r1, dx)                            # MATLAB gradient(r1,dx)
    dA = 2 * np.pi * r1 * np.sqrt(1 + drdx**2) * dx
