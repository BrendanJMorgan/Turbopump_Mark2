...
function curve = bezier(points, num_points) % Generates a quartic bezier curve for four inputted control points. 
					
	binomialCoefficients = [1, 3, 3, 1];
   
    bezierPoints = []; % Initialize an empty array to store the Bezier curve points
    
    for u = 0:1/(num_points-1):1 % Loop through parameter 'u' from 0 to 1 to generate points on the Bezier curve
