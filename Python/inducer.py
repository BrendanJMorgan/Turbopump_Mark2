...
def inducer(p) -> None:
    ...
    p.cavitation_limit = (p.shaft_speed * p.r_inlet_impeller) ** 2 / (2 * engine.g)  # unitless
    ...
    p.NPSH_available = (engine.p_amb - p.pvap_inlet) / (p.density * engine.g)  # m - Net positive suction head. Assuming zero plumbing water column and inlet velocity
    ...
    p.r_tip_inducer = 0.5 * 0.3048 * 0.37843 * (
        (p.vdot_corrected * 15580.323) / (p.shaft_speed * 30 / np.pi * p.flow_coeff_inducer)
    ) ** (1 / 3)  # m
