...
class engine: # For parameters that don't fit neatly within other categories
    ...
    thrust = 40000      # N - Thrust INCLUDE GG EXHAUST THRUST
    ...
    shaft_speed_cap = 20000 * np.pi/30 # rad/s - maximum shaft speed for the pumps if they are not otherwise suction limited
...
class inducer:
    def __init__(self, fluid: str):
        self.fluid = fluid
        if self.fluid == 'ox':
            self.flow_coeff = 0.06         # unitless
            self.blade_number = 3           # unitless
            self.clearance_radial = 2E-4    # m
            self.hub_tip_ratio = 0.2        # unitless - ratio of hub radius to tip radius of inducer
            self.hub_tip_ratio_out = 0.5    # unitless - ratio of hub radius to tip radius of inducer
            self.suction_margin_factor = 1.5       # unitless - Margin of extra net positive suction head (NPSH)
            self.flow_margin_factor = 1.2          # unitless - margin on flow rate to ensure inducer is not undersized
            ...
            self.sweepback = (65+90)/2*np.pi/180 # rad - angle of sweepback of blade eading edges
        elif self.fluid == 'fuel':
            self.flow_coeff = 0.15          
            self.blade_number = 3
            self.clearance_radial = 2E-4
            self.hub_tip_ratio = 0.2
            self.hub_tip_ratio_out = 0.5   
            self.suction_margin_factor = 1.5
            self.flow_margin_factor = 1.2      
...
class impeller:
    def __init__(self, stage: int, fluid: str):
        ...
        if self.fluid == 'ox':
            ...
            self.suction_margin = 1.5
            self.surface_roughness = 10E-6
            self.plumbing_loss = 20*6894.76 # Pa
...
class volute:
    def __init__(self, fluid: str):
        ...
        if self.fluid == 'ox':
            self.dummy = 1.0   
            self.wrap_angle = 2*np.pi       # rad - single volute is 2*pi, double volute is pi, etc.                
        elif self.fluid == 'fuel':
            self.dummy = 1.0    
            self.wrap_angle = 2*np.pi       # rad - single volute is 2*pi, double volute is pi, etc.            

class turbine: 
    r_pitchline = 4.5*0.0254                # m
