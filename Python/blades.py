...
def blades(p: pump):
    # Meanline Definition
    # r_inlet_blade = (p.r_hub_impeller + p.r_inlet_impeller) * 0.75 # m - radial location of blade index start - typically vertically aligned with eye radius - this assumption can be changed
    # inlet_idx = int(np.argmax(p.meanline_curve[:, 0] > r_inlet_blade))
    inlet_idx = int(0.1*p.meanline_curve.shape[0]) # index where blades start
    ...
    blade_angle = np.arctan2(p.v_merid, (p.u_blade * (1.0 - p.slip_factor) - v_tangential)) # rad - angle between blade tip azimuth and local tangential azimuth (beta_2 in pump handbook)
    ...
    theta_blade = cumulative_trapezoid(1.0/np.tan(blade_angle)/p.meanline_curve_bladed[:,0], p.meanline_curve_bladed[:,0], initial=0)  # rad

    p.blade_curve = np.column_stack([p.meanline_curve_bladed[:,0]*np.cos(theta_blade),p.meanline_curve_bladed[:,0]*-p.clocking*np.sin(theta_blade),p.meanline_curve_bladed[:,1],])

    # Number of Blades -------------------------------------------------------------
    blade_arc_length = float(np.linalg.norm(np.diff(p.blade_curve, axis=0), axis=1).sum())
    ...
    test = solidity_ideal * (2.0 * np.pi * p.r_outlet_impeller) / blade_arc_length
    ...
    p.meridional_length = np.sum(np.sqrt(np.diff(p.blade_curve[:, 0])**2 + np.diff(p.blade_curve[:, 2])**2)) # m - arc length of blade projected onto the meridional plane
