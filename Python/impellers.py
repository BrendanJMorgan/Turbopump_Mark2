...
def impellers(p: pump):
    ...
    imp.area_meanline = np.pi * (imp.shroud_curve[:,0] + imp.hub_curve[:,0]) * gap * imp.blockage # m2 - annular flow area along meanline
    ...
    imp.area_meanline[np.isnan(imp.area_meanline)] = PchipInterpolator(np.where(~np.isnan(imp.area_meanline))[0], imp.area_meanline[~np.isnan(imp.area_meanline)])(np.where(np.isnan(imp.area_meanline))[0]) # m2
    imp.gap_meanline = (imp.shroud_curve[:,0] / normals_meanline[:,0]) * (np.sqrt(1 + (imp.area_meanline*normals_meanline[:,0]) / (np.pi*imp.shroud_curve[:,0]**2)) - 1) # m - crosswise gap along meanline
    ...
    imp.v_merid = imp.vdot / imp.area_meanline # m/s - meridional velocity along meanline
    imp.flow_coeff = imp.v_merid / (p.shaft_speed * imp.meanline_curve[:,0]) # unitless - flow coefficient along meanline
